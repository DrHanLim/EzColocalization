...
public class HistogramStackWindow extends StackWindow implements Measurements, 
ClipboardOwner, ImageListener, RoiListener, ActionListener, Runnable {
	...
	public HistogramStackWindow(ImagePlus imp) {
		...
		//updateImage(createRGBImage("Histogram of "+imp.getShortTitle(), imp));
	}
	...
	/*public HistogramStackWindow(String title, ImagePlus imp, ImageStatistics stats) {
	...
	private void syncStack(ImagePlus imp)
	{
		...
		if (stackSize==1) {
			String label = impStack.getSliceLabel(1);
 			if (label!=null) this.imp.setProperty("Label", label);
 			withScrollBar=false;
 		}
	...
	{
		ImagePlus rimp = null;
		rimp = IJ.createHyperStack(title, WIN_WIDTH, WIN_HEIGHT, imp.getNChannels(), imp.getNSlices(), imp.getNFrames(), 24);
		rimp.resetStack();
		return rimp;
	}
	...
	public void showHistogram(ImagePlus imp, int bins, double histMin, double histMax) {
		...
		for(int iSlice=1;iSlice<=impStack.getSize();iSlice++){
			if (channel!=INTENSITY && imp.getType()==ImagePlus.COLOR_RGB) {
				...
				stats = imp2.getStatistics(AREA+MEAN+MODE+MIN_MAX+(limitToThreshold?LIMIT:0), bins, histMin, histMax);
			}
	...
	public void setup() {if(list==null)setup(imp);}
	...
	void drawAlignedColorBar(ImagePlus imp, int iSlice, double xMin, double xMax, ImageProcessor ip, int x, int y, int width, int height) {
	...
	public void run() {
		while (!done) {
			...
			if (doUpdate && srcImp!=null && Thread.currentThread()==bgThread) {
				...
				if (srcImp!=null) {
					if(srcImp.getNChannels()!=imp.getNChannels()||
					   srcImp.getNFrames()!=imp.getNFrames()||
